...
<?php
    ...
    if ( !empty($_POST)) {
        ...
        $sql = "DELETE FROM receta  WHERE receta_id = ?";
        $q = $pdo->prepare($sql);
        $q->execute(array($id));
        Database::disconnect();
        echo "
     ...
     <?php 
      ...
      { ?>
                <div class="span10 offset1">
                    ...
                    <form class="form-horizontal" action="deleteReceta.php" method="post">
                      <input type="hidden" name="id" value="<?php echo $id;?>"/>
                      <p class="alert alert-error">A jeni i sigurt qe deshironi te fshini kete recete?</p>
                      <div class="form-actions">
                          <button type="submit" class="btn btn-danger">Po, vazhdo.</button>
                          <a class="btn" href="user_receta.php">Jo, me kthe mbrapa.</a>
                        </div>
                    </form>
                </div>
                 <?php } ?>
